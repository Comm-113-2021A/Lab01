...
subtitle: "Due: February 2, 4pm EST time"
name: "Your Name"
output: 
  tufte::tufte_html:
    css: lab.css
...
-   **Repository:** A Git repository contains all of your project's files and stores each file's revision history. It's common to refer to a repository as a repo.
    ...
    -   For individual assignments, only you will have access to the repo. For team assignments, all team members will have access to a single repo where they work collaboratively. Since I want everyone to have practice this time with git, everyone should submit this lab as an individual assignment.
    -   All repos associated with this course are housed in a GitHub organization: [`Comm-113-2021A`](https://github.com/Comm-113-2021A/). The organization is set up such that students can only see repos they have access to, but the course staff can see all of them.
...
-   are a member of the course GitHub organization
    ...
    -   You can check this by going to  [`Comm-113-2021A`](https://github.com/Comm-113-2021A/) and confirming that you can see a repository called `lab-01-pet-names-YOUR_GITHUB_USERNAME` where `YOUR_GITHUB_USERNAME` is, well, your github username.

        ```{r rstudio-cloud, fig.margin = TRUE, echo = FALSE, eval = TRUE}
        knitr::include_graphics("img/cloudspace.png")
        ```

-   are a member of the course RStudio Cloud space

    -   You can check this by going to [rstudio.cloud](https://rstudio.cloud/ "RStudio Cloud") and confirming that you can see Comm 113 listed as one of your workspaces on the left menu.

If you believe you've completed all of the prerequisites and you are still having difficulty, review troubleshooting advice [here](https://www.introds.org/troubleshoot/github-org.html).

**!!!IMPORTANT!!!** Your repository is not the one called `lab` or `homework', that's where I write the lab and homework instructions (like this document!).
You should look for a repo with your username, and if it doesn't exist, go back and do the prerequisite steps.
If you have done them, and it still doesn't exist, then let me know and I'll help you get unstuck.
...
✏️ *Write your answer in your R Markdown document under Exercise 1, knit the document, commit your changes with a commit message that says "Completed Exercise 1", and push.*

1.  Again, according to the data dictionary, how many variables do we have for each pet?

✏️ *Write your answer in your R Markdown document under Exercise 2, knit the document, commit your changes with a commit message that says "Completed Exercise 2", and push.*

1.  What are the three most common pet names in Seattle? To do this you will need to count the frequencies of each pet name and display the results in descending order of frequency so that you can easily see the top three most popular names. The following code does exactly that.
